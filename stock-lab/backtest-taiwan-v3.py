...
model-status manifest and frozen validation contracts, then emits the same non-PASS
states. A PASS may only be written by a separate formal workflow that receives
legally reusable historical bundles and runs the production-equivalent formula
chronologically OOS.
"""
import argparse
...
def validate_holding_protocol(x):
    assert x.get('schema_version') == 1, 'holding protocol schema mismatch'
    assert x.get('protocol_name') == 'StockLab TW-holding-exit-v3 OOS Protocol'
    assert x.get('frozen_before_historical_oos') is True
    rules=x.get('non_negotiable_rules') or {}
    for k in (
        'no_horizon_selector','holding_age_drives_structure','cost_basis_is_position_specific',
...
def validate_status_manifest(x,holding_protocol_path):
    ...
    assert holding.get('candidate_formula')=='TW-holding-exit-v3'
...
def main():
    ...
    print(json.dumps({
        ...
        'holding_protocol_frozen_and_validated':out['holding_protocol_frozen_and_validated'],
        'models': out['models']
