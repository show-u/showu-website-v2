...
(function(){
  ...
  if(hold){const original=hold.onclick;hold.onclick=async function(ev){
    ...
    const box=document.querySelector('#holdResult');try{
      ...
      const pnl=(x.close-p.averageCost)*p.shares,pct=p.averageCost>0?(x.close/p.averageCost-1)*100:null;
      box.innerHTML=`<div class=toprow><div><h2>${esc(x.name)}／${esc(x.code)}</h2><div class=muted>${esc(x.market)}｜已持有</div></div></div><div class=sourcegrid style="margin-top:10px"><div class=sourceitem><b>成本均價</b>${money(p.averageCost)}</div><div class=sourceitem><b>持有股數</b>${money(p.shares)}</div><div class=sourceitem><b>最新已驗證收盤</b>${esc(x.date||'—')}｜${money(x.close)}</div><div class=sourceitem><b>未實現損益</b>${pnl>=0?'+':''}${money(pnl)}${pct!=null?`｜${pct>=0?'+':''}${pct.toFixed(2)}%`:''}</div></div><div class=source-note><b class=bad>暫不提供出場價格與信心指數</b><div class=mini>持股出場模型的正式 OOS 與信心校準尚未通過；目前只顯示你的持股事實與已驗證收盤，不捏造賣出價。</div></div>`;
