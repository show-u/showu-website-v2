...
(function(){
  ...
  function ready(){
    if(!promise)promise=fetch(URL,{cache:'no-store'}).then(async r=>{if(!r.ok)throw Error('verification-policy unavailable');const x=await r.json();if(x.version<4)throw Error('verification-policy version too old');POLICY=x;return x;});
    return promise;
  }
  function ok(x){return x===true}
  ...
  function req(mode,a,vf,ctx={}){
    ...
    if(ctx.corporateActionKnown===false)warnings.push('公司行動／參考價事件尚未完整驗證');
    if(mode==='preopen'){
      if(!ctx.sufficientHistory)blockers.push('盤前模型歷史 OHLC 樣本不足');
      if(ctx.preopenSessionValid===false)blockers.push('已超過開盤前委託有效時段');
      if(ctx.eventFeedAvailable===false)warnings.push('重大事件自動資料流未接入；不得把未知寫成「無事件」');
    }else if(mode==='1m'){
      if(!q.institution?.ok)blockers.push('三大法人資料缺漏／日期不一致');
      if(!q.revenue?.ok)blockers.push('月營收資料缺漏／期別過舊');
    }else if(mode==='3m'){
      if(!q.institution?.ok)blockers.push('三大法人資料缺漏／日期不一致');
      if(!q.revenue?.ok)blockers.push('月營收資料缺漏／期別過舊');
      if(!q.quarterly?.ok)blockers.push('季報資料缺漏／期別不合格');
    }else if(mode==='long'){
      if(!q.valuation?.ok)blockers.push('估值資料缺漏／日期不合格');
      if(!q.revenue?.ok)blockers.push('月營收資料缺漏／期別過舊');
      if(!q.quarterly?.ok)blockers.push('季報資料缺漏／期別不合格');
    }
    if(ctx.modelValidationStatus&&ctx.modelValidationStatus!=='PASS')warnings.push(`模型樣本外驗證：${ctx.modelValidationStatus}；不得標示為已驗證推薦`);
  ...
  function resultLabel(validationStatus){return validationStatus==='PASS'?'已通過樣本外驗證的模型參考':'研究模型估計（未取得有效樣本外 PASS 不得視為推薦）'}
