...
(function(){
  ...
  const ENTRY_FACTOR_KEYS=['priceVolume','taiwanMarket','technical','chips','valuation','revenue','quarterly','financialQuality','industryTrend'];
  function ready(){if(!promise)promise=fetch(URL,{cache:'no-store'}).then(async r=>{if(!r.ok)throw Error('verification-policy unavailable');const x=await r.json();if(x.version<14)throw Error('verification-policy version too old');POLICY=x;return x;});return promise}
