...
// User-observed position facts + licensed/verified market facts only. No local sell recommendation is fabricated.
(function(){
  ...
  async function readinessStatus(){try{const r=await fetch('./holding-readiness.json',{cache:'no-store'});if(!r.ok)throw Error();const x=await r.json();if(![1,2,3].includes(x.schema_version)||x.model!=='TW-holding-exit-v4')throw Error();return x}catch{return null}}
  function readinessHtml(x){
    ...
    const h=x.history||{},o=x.oos||{},src=x.selected_history_source||{},act=x.activation_path||{},ok=x.overall_status==='PASS'&&x.executable_exit_output===true,blockers=Array.isArray(x.blockers)?x.blockers:[];
    const sourceLabel=src.kind==='licensed_history_bundle'?`授權歷史 bundle${src.provider?`｜${esc(src.provider)}`:''}`:'OGDL 合法每日向前累積（僅備援，不作歷史回補）';
    const activation=act.primary==='licensed_historical_backfill'?`正式解法：授權歷史 backfill｜${act.status==='pending_licensed_subscription_or_import'?'待完成授權訂購／私有匯入':'狀態 '+esc(act.status||'UNKNOWN')}`:'正式解法狀態未取得';
    const b=blockers.length?`<br>${blockers.map(z=>`• ${esc(z)}`).join('<br>')}`:'';
    return `<div class=source-note><b>${ok?'✅':'⚠️'} 正式出場模型準備度｜${esc(x.overall_status||'UNKNOWN')}</b><div class=mini>${activation}<br>目前選用歷史來源：${sourceLabel}<br>歷史覆蓋：單檔最多 ${money(h.max_valid_bars_per_security)}/${money(h.minimum_valid_bars)} 根；達門檻標的 ${money(h.securities_at_least_minimum)} 檔。<br>OOS：${esc(o.status||'UNKNOWN')}｜episodes ${money(o.episodes)}/${money(o.minimum_episodes)}｜securities ${money(o.securities)}/${money(o.minimum_securities)}。<br>可執行性 ${o.execution_validated===true?'PASS':'未通過'}｜信心校準 ${o.confidence_calibrated===true?'PASS':'未通過'}｜production/test formula ${o.production_formula_match===true?'一致':'未證明一致'}。${b}</div></div>`;
  }
  async function run(){
    ...
    try{
      ...
      const forwardText=hist?`${hist.first||'—'}～${hist.last||'—'}｜有效交易日 ${hist.dates??'—'}｜單檔最多 ${hist.maxBars??'—'} 根`:'OGDL 向前累積狀態未取得';
      box.innerHTML=`<div class=toprow><div><h2>${esc(x.name)}／${esc(x.code)}</h2><div class=muted>${x.market}｜已持有｜持股監控</div><div class=mini>市場資料日 ${esc(x.date)}｜合法公開最新收盤｜缺值不補</div></div></div><div class=decision-strip><div class=sourceitem><div class=hero-label>依最新合法收盤的未實現損益</div><div class=hero-number>${pnl>=0?'+':''}${money(pnl)}</div><div class=mini>${pct(pnlPct)}｜未計你未輸入的手續費、交易稅、股利與已實現損益</div></div><div class=sourceitem><div class=hero-label>最新已驗證收盤</div><div class=hero-number>${money(x.close)}</div><div class=mini>${esc(x.date)}｜完成交易日收盤，不是盤中即時價</div></div></div><div class=sourcegrid style="margin-top:8px"><div class=sourceitem><b>成本均價</b>${money(pos.averageCost)}</div><div class=sourceitem><b>目前持有股數</b>${money(pos.shares)}</div><div class=sourceitem><b>持倉總成本</b>${money(cost)}</div><div class=sourceitem><b>依收盤估算市值</b>${money(value)}</div><div class=sourceitem><b>首次買入日</b>${esc(pos.buyDate)}${calDays!=null?`｜至資料日 ${calDays} 個日曆日`:''}<br><span class=mini>不是交易日數；正式模型只使用驗證過的實際交易日</span></div><div class=sourceitem><b>當日價格事實</b>${esc(dayFact(x))}</div><div class=sourceitem><b>台股風險事實</b>${esc(riskFact(factors,x))}</div><div class=sourceitem><b>最新市場資料來源</b>licensed-open-data-cache｜OGDL 1.0</div></div>${readinessHtml(ready)}<div class=source-note><b>正式出場建議目前為什麼沒有數字？</b><div class=mini>正式持股模型最低需要 120 根有效日線；主要路徑是「私有授權歷史 backfill → importer 驗證 → production-equivalent OOS → 僅發布不含 raw rows 的驗證摘要」。不是等待 OGDL 累積滿 120 天。OGDL 只負責每日合法向前增量備援。尚未匯入授權歷史 bundle 前，正式 OOS 保持 INSUFFICIENT，不輸出假的賣出時機。OGDL forward archive：${esc(forwardText)}。</div></div><div class=disclaimer><b>硬規則</b>你的成本、股數、買入日只採你的輸入；市場事實只採授權與驗證通過來源。任何未取得資料維持「未知」，不以 0、平均值、舊值、其他網站或 AI 補成市場事實。</div>`;
