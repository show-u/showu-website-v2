...
(function(){
  ...
  async function readinessStatus(){try{const r=await fetch('./holding-readiness.json',{cache:'no-store'});if(!r.ok)throw Error();const x=await r.json();if(![1,2].includes(x.schema_version)||x.model!=='TW-holding-exit-v4')throw Error();return x}catch{return null}}
  function readinessHtml(x){
    ...
    const h=x.history||{},o=x.oos||{},src=x.selected_history_source||{},ok=x.overall_status==='PASS'&&x.executable_exit_output===true,blockers=Array.isArray(x.blockers)?x.blockers:[];
    const sourceLabel=src.kind==='licensed_history_bundle'?`授權歷史 bundle${src.provider?`｜${esc(src.provider)}`:''}`:'OGDL 合法每日向前累積（fallback）';
    const b=blockers.length?`<br>${blockers.map(z=>`• ${esc(z)}`).join('<br>')}`:'';
    return `<div class=source-note><b>${ok?'✅':'⚠️'} 正式出場模型準備度｜${esc(x.overall_status||'UNKNOWN')}</b><div class=mini>正式歷史來源：${sourceLabel}<br>歷史覆蓋：單檔最多 ${money(h.max_valid_bars_per_security)}/${money(h.minimum_valid_bars)} 根；達門檻標的 ${money(h.securities_at_least_minimum)} 檔。<br>OOS：${esc(o.status||'UNKNOWN')}｜episodes ${money(o.episodes)}/${money(o.minimum_episodes)}｜securities ${money(o.securities)}/${money(o.minimum_securities)}。<br>可執行性 ${o.execution_validated===true?'PASS':'未通過'}｜信心校準 ${o.confidence_calibrated===true?'PASS':'未通過'}｜production/test formula ${o.production_formula_match===true?'一致':'未證明一致'}。${b}</div></div>`;
  }
  async function run(){
