// International event policy. Observation only; no headline can change scores.
const EVENT_POLICY={
 A:{label:'A｜已驗證重大事件',rule:'政府／監管機關正式文件、已生效規則、公司正式重大公告或財報。'},
 B:{label:'B｜高度可信',rule:'公司 IR，或兩個彼此獨立的高品質媒體對同一事件相互印證。'},
 C:{label:'C｜暫定觀察',rule:'單一可信媒體報導；必須標示發布日與事件日，不得進分數。'},
 D:{label:'D｜排除',rule:'社群、論壇、匿名消息、未交叉驗證傳聞或單純評論。'}
};
function eventPolicyPanel(){return `<h3>事件資料規則</h3><div class=sourcegrid>${Object.values(EVENT_POLICY).map(x=>`<div class=sourceitem><b>${x.label}</b><span class=mini>${x.rule}</span></div>`).join('')}</div><div class=mini style="margin-top:8px">目前沒有通過完整 Gate 的自動事件資料流，因此不顯示「今日重大事件」清單，也不改變任何股票分數。</div>`}
