...
(function(){
  ...
  const totalCostInput=document.querySelector('#holdTotalCost');
  const buyTime=document.querySelector('#holdBuyTime');
  const holdResult=document.querySelector('#holdResult');
  if(!mode||!manual||!lotsBox||!rows||!addBtn||!summary||!avgCost||!shares||!totalCostInput||!buyTime)return;
  ...
  function recompute(){
    if(mode.value!=='lots'){summary.innerHTML='<b>逐筆明細未啟用</b><div class="mini">目前以你輸入的買入時間、持股數，以及「成本均價／部位總成本」其中一個為準；另一個只做確定性換算。</div>';return}
    ...
    avgCost.value=String(+weighted.toFixed(4));shares.value=String(+totalShares.toFixed(4));totalCostInput.value=String(+sumCost.toFixed(4));buyTime.value=times[0]||'';
  ...
  function collect(){
    if(mode.value==='manual'){
      let a=num(avgCost.value),sh=num(shares.value),tc=num(totalCostInput.value),raw=buyTime.value||'',bt=validDateTime(raw)?raw:null;
      if(!(sh>0)||!bt)throw Error('請完整輸入首次買入時間與目前持有股數');
      if(!(a>0)&&!(tc>0))throw Error('成本均價與部位總成本至少要輸入一個');
      if(a>0&&tc>0){const derived=a*sh,tol=Math.max(1,Math.abs(tc)*0.001);if(Math.abs(derived-tc)>tol)throw Error('成本均價 × 股數與你輸入的總成本不一致，請確認後再分析')}
      if(!(a>0)){a=tc/sh;avgCost.value=String(+a.toFixed(4))}
      if(!(tc>0)){tc=a*sh;totalCostInput.value=String(+tc.toFixed(4))}
      return{mode:'manual',averageCost:a,shares:sh,totalCost:tc,buyTime:bt,buyDate:bt.slice(0,10),lots:null,positionComplete:true,provenance:{averageCost:num(avgCost.value)>0?'user_or_derived':'derived_from_total_cost',shares:'user_observed',totalCost:num(totalCostInput.value)>0?'user_or_derived':'derived_from_average_cost',buyTime:'user_observed'}};
    }
  ...
  function provenanceHtml(){return mode.value==='lots'?'<b>部位資料來源｜derived from user input</b><div class="mini">總股數、總成本、加權均價與最早買入時間由你輸入的逐筆明細確定性計算；不是市場推估。</div>':'<b>部位資料來源｜user observed + derived</b><div class="mini">首次買入時間與股數採你的輸入；成本均價／部位總成本可擇一輸入，另一個只由股數做確定性換算。若兩者都填，必須先通過一致性檢查。</div>'}
