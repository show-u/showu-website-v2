...
(function(){
  ...
  function recompute(){
    if(mode.value!=='lots'){summary.innerHTML='<b>逐筆明細未啟用</b><div class="mini">目前以你輸入的首次買入時間、持股數與成本均價為準；總成本固定由「均價 × 股數」確定性計算。</div>';return}
  ...
  function collect(){
    if(mode.value==='manual'){
      ...
      if(!(a>0)||!(sh>0)||!bt)throw Error('請完整輸入成本均價、目前持有股數與首次買入時間');
      const tc=a*sh;
      return{mode:'manual',averageCost:a,shares:sh,totalCost:tc,buyTime:bt,buyDate:bt.slice(0,10),lots:null,positionComplete:true,provenance:{averageCost:'user_observed',shares:'user_observed',totalCost:'derived_from_average_cost_times_shares',buyTime:'user_observed'}};
    }
  ...
  function provenanceHtml(){return mode.value==='lots'?'<b>部位資料來源｜derived from user input</b><div class="mini">總股數、總成本、加權均價與最早買入時間由你輸入的逐筆明細確定性計算；不是市場推估。</div>':'<b>部位資料來源｜user observed + derived</b><div class="mini">首次買入時間、股數與成本均價採你的輸入；總成本固定由「成本均價 × 目前持有股數」確定性計算。</div>'}
