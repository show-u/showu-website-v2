// Taiwan data integrity layer: validates source dates/periods before scoring. Unknown stays unknown.
...
function quarterFresh(q){if(!q||q.completeStatements!==true||q.samePeriod!==true)return false;const rd=parseTWDate(q.reportDate);if(!rd)return false;const age=(Date.now()-rd.getTime())/86400000;return age>=-2&&age<=220}
function factorIntegrity(stockDate,v,i,r,q){const valuationDate=v?.date||null,revPeriod=normalizePeriod(r?.period),minRev=expectedRevenueMinPeriod(),instDate=i?.date||null;return{
 price:{ok:!!stockDate,date:stockDate,reason:stockDate?'價格交易日存在；仍須通過行情交叉驗證':'缺少交易日'},
 institution:{ok:!!i&&!!instDate&&dateGap(instDate,stockDate)===0,date:instDate,reason:!i?'資料未取得／來源不可用，不計分':!instDate?'來源無可驗證日期，不計分':dateGap(instDate,stockDate)===0?'法人日期與價格日一致':'法人日期不一致，不計分'},
 valuation:{ok:!!v&&!!valuationDate&&dateGap(valuationDate,stockDate)<=7,date:valuationDate,reason:!v?'未取得估值':!valuationDate?'估值缺少日期，不計分':dateGap(valuationDate,stockDate)<=7?'估值日期在容許範圍':'估值日期過舊或不一致'},
 revenue:{ok:!!r&&!!revPeriod&&revPeriod>=minRev,date:revPeriod,reason:!r?'未取得月營收':revPeriod&&revPeriod>=minRev?'月營收期別符合最新應公布範圍':`月營收期別過舊／無效，最低應為 ${minRev}`},
 quarterly:{ok:quarterFresh(q),date:q?`${q.year||'—'}Q${q.quarter||'—'} / ${q.reportDate||'—'}`:null,reason:!q?'未取得季報':q.completeStatements!==true?'損益表／資產負債表不完整或期別不一致，不計分':quarterFresh(q)?'同期間完整季報且出表日有效':'季報出表日過舊或異常'}
}}
// Never assign the request date to a record whose source did not provide a date.
extras=async function(date){return _integrityExtras(date)};
combine=function(a,h,m,v,i,r){a=_integrityCombine(a,h,m,v,i,r);const integ=factorIntegrity(a.last?.iso,v,i,r,a.fin);let correction=0;
 ...
 a.integrity=integ;a.integrityCorrection=correction;a.score=Math.max(0,Math.min(100,a.score+correction));a.action=a.score>=75?'研究訊號偏多｜等待驗證與風險條件':a.score>=60?'研究訊號觀察｜不追價':a.score>=45?'研究訊號中性｜等待確認':'研究訊號偏弱｜暫避';return a};
function integrityPanel(a){const q=a.integrity||{},row=(name,x)=>`<div class=sourceitem><b>${x?.ok?'✅':'⚠️'} ${name}</b>${x?.date||'—'}<br><span class=mini>${x?.reason||'未驗證'}</span></div>`;return `<h3>資料完整性</h3><div class=sourcegrid>${row('價格',q.price)}${row('三大法人',q.institution)}${row('估值',q.valuation)}${row('月營收',q.revenue)}${row('季報',q.quarterly)}<div class=sourceitem><b>計分修正</b>${a.integrityCorrection>0?'+':''}${a.integrityCorrection||0}<br><span class=mini>未通過來源／日期／期別驗證的因子已退出分數；缺資料不補值</span></div></div>`}
