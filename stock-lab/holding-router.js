...
(function(){
  ...
  async function legalBars(code,market,snap){
    try{const h=window.StockLabLicensedHistory;if(h?.load){const bars=await h.load({code,market,minimumBars:1});if(Array.isArray(bars)&&bars.length)return{bars,source:'licensed-history'}}}catch{}
  ...
  async function context(code,market){
    ...
    try{if(window.StockLabTaiwan?.loadFactors){await window.StockLabTaiwan.loadFactors();const f=window.StockLabTaiwan.stockFactor?.(code)||{};riskKnown=Object.prototype.hasOwnProperty.call(f,'disposition')||Object.prototype.hasOwnProperty.call(f,'attention')||Object.prototype.hasOwnProperty.call(f,'margin_suspended');riskBlocked=f.disposition===true||f.margin_suspended===true;riskLabel=riskKnown?(f.disposition?'處置':f.attention?'注意':f.margin_suspended?'信用交易受限':'已驗證無旗標'):'注意／處置狀態未完整驗證'}}catch{}
    try{const h=window.StockLabLicensedHistory;if(h?.context){await h.context({code,market});corporateKnown=true}}catch{}
    return{riskKnown,riskBlocked,riskLabel,corporateKnown};
  }

  async function sessionInfo(market,dataDate){try{const s=await window.StockLabSessionContext?.resolve?.({market,dataDate});return s?.verified?`${s.label||''}${s.targetDate?`｜下一合法交易日 ${s.targetDate}`:''}`:'下一合法交易日尚未驗證'}catch{return'下一合法交易日尚未驗證'}}
