...
(function(){
  ...
  function render(code,name,market,p,snap,res,ctx,barSource,session){
    ...
    box.innerHTML=positionFacts(code,name,p)+
      ...
      `<div class=source-note><b>執行時間</b><div class=mini>${esc(session)}。本頁用完成交易日資料形成條件；沒有合法即時行情時，不宣稱知道盤中現在價格。</div></div><div class=disclaimer><b>這不是 OOS 鎖價頁</b>持股的成本、股數、買入時間、最新合法已驗證收盤、損益與下一交易時段執行定價基準不受 OOS、120 根、300 episodes 或 80 檔門檻鎖住。OOS 只限制勝率、成功率、機率與另外的預測型出場模型。</div>`;
  }
