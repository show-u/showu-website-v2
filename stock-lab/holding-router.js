...
(function(){
  ...
  function localExitReference(result){
    ...
    if(state.includes('分批停利'))return{label:'停利檢視價',value:d.resistance,condition:'接近結構壓力且趨勢轉弱時，檢視分批停利'};
    if(state.includes('獲利保護'))return{label:'獲利保護價',value:d.profitDefense,condition:'完成交易日收盤跌破後，下一合法交易時段重新確認減碼／出場'};
    return{label:'出場防守價',value:d.defense,condition:'完成交易日收盤跌破且結構同步轉弱時，下一合法交易時段重新確認出場'};
  }
  async function localHolding(code,p,name,market){
    ...
    const d=result.derived||{},q=result.decision||{},ref=localExitReference(result),pnlPct=Number(d.pnlPct),pnlText=Number.isFinite(pnlPct)?`${money(d.pnl)}（${pnlPct.toFixed(2)}%）`:MISSING;
    box.innerHTML=positionFacts(code,name,p)+`<div class=decision-strip><div class=sourceitem><div class=hero-label>${esc(ref.label)}</div><div class=hero-number>${Number.isFinite(Number(ref.value))?money(ref.value):MISSING}</div><div class=mini>${esc(ref.condition)}</div></div><div class=sourceitem><div class=hero-label>信心指數</div><div class=hero-number>${Math.round(ci)}</div><div class=mini>/100｜已校準；不是成交機率</div></div></div><div class=sourcegrid style="margin-top:8px"><div class=sourceitem><b>最新官方收盤</b>${esc(result.dataDate)}｜${money(result.latestClose)}</div><div class=sourceitem><b>未實現損益</b>${pnlText}</div></div><div class=source-note><b>${esc(q.state||'持股判斷')}</b><div class=mini>${esc(q.reason||'')}</div></div>`;
  }
  function renderPrivate(code,p,j){
    ...
    box.innerHTML=facts+`<div class=decision-strip><div class=sourceitem><div class=hero-label>建議出場價格${hasRange?'區間':''}</div><div class=hero-number>${exitText}</div><div class=mini>${esc(d.exit_condition||'依模型條件於下一合法交易時段重新確認')}</div></div><div class=sourceitem><div class=hero-label>信心指數</div><div class=hero-number>${Math.round(ci)}</div><div class=mini>/100｜已校準；不是成交機率</div></div></div><div class=sourcegrid style="margin-top:8px"><div class=sourceitem><b>最新官方收盤</b>${esc(d.data_date||'—')}${d.close!=null?`｜${money(d.close)}`:''}</div><div class=sourceitem><b>未實現損益</b>${pnl}｜${pct}</div></div><div class=source-note><b>${esc(d.action||'持股判斷')}</b><div class=mini>${esc(d.reason||'')}</div></div>`
  }
  btn.onclick=async()=>{
