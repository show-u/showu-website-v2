...
(function(){
  ...
  function render(code,name,market,p,snap,res,ctx,barSource,session){
    ...
    box.innerHTML=positionFacts(code,name,p)+
      ...
      `<h3>持股價格與 9+3 出場決策</h3><div class=sourcegrid><div class=sourceitem><b>下一交易時段出場／減碼定價基準</b>${executionText}<br><span class=mini>${esc(q.executionReferenceMeaning||'最新完成交易日收盤基準')}</span></div><div class=sourceitem><b>成本損益零界</b>${breakevenText}<br><span class=mini>${esc(q.breakevenMeaning||'使用者成本均價')}</span></div><div class=sourceitem><b>結構風險線</b>${triggerText}<br><span class=mini>${esc(q.riskTriggerMeaning)}</span></div><div class=sourceitem><b>${w.complete?'9+3 完整加權分數':'已驗證部分方向分數'}</b>${q.decisionScore??'—'}/100<br><span class=mini>${w.complete?'12項全部通過驗證後的持有方向':'只計已驗證項目；不可當成完整 9+3 結論'}；不是上漲機率</span></div><div class=sourceitem><b>出場決策信心指數</b>${q.decisionConfidenceIndex??'—'}/100<br><span class=mini>${esc(q.confidenceMeaning||'資料不足')}</span></div><div class=sourceitem><b>9+3 資料覆蓋</b>${w.coveragePct??0}%<br><span class=mini>${missing?'未驗證：'+esc(missing):'9+3 全部通過驗證'}</span></div></div>`+
      `<div class=source-note style="margin-top:8px"><b>建議出場／減碼條件</b><div style="margin-top:6px"><b>出場檢視：</b>${esc(q.exitCondition||'無法判定')}</div><div style="margin-top:6px"><b>減碼／停利檢視：</b>${esc(q.reduceCondition||'無法判定')}</div><div class=mini style="margin-top:6px">下一交易時段定價基準只來自最新合法已驗證完成交易日收盤；它不是盤中即時價或預測成交價。結構風險線另由合法已驗證 OHLC 形成，資料不足就維持「—」，兩者不得混為一談。</div></div>`+
      `<div class=sourcegrid style="margin-top:8px"><div class=sourceitem><b>最新已驗證收盤</b>${money(snap.close)}${snap.date?`｜${esc(snap.date)}`:''}<br><span class=mini>${esc(snap.source)}｜非盤中即時價</span></div><div class=sourceitem><b>依收盤估算損益</b>${profit>=0?'+':''}${money(profit)}｜${pct>=0?'+':''}${Number.isFinite(pct)?pct.toFixed(2):'—'}%</div><div class=sourceitem><b>買入後價格經歷</b>${money(d.tradingBarsSinceBuy)} 根交易日K<br><span class=mini>期間高點 ${money(d.sinceEntryHigh)}｜低點 ${money(d.sinceEntryLow)}｜最大浮盈 ${Number.isFinite(Number(d.maxGainPct))?Number(d.maxGainPct).toFixed(2)+'%':'—'}｜最大回落 ${Number.isFinite(Number(d.maxDrawdownPct))?Number(d.maxDrawdownPct).toFixed(2)+'%':'—'}</span></div><div class=sourceitem><b>停利／壓力參考</b>${pressureText}<br><span class=mini>${esc(q.pressureMeaning)}</span></div><div class=sourceitem><b>可用價格資料</b>${esc(d.historyLevel||'—')}｜${money(d.availableBars)} 根<br><span class=mini>${esc(barSource)}</span></div><div class=sourceitem><b>台股特殊狀態</b>${esc(ctx.riskLabel)}<br><span class=mini>${ctx.corporateKnown?'公司行動資料層已取得':'公司行動狀態未完整驗證'}</span></div><div class=sourceitem><b>價格形成依據</b>${esc(q.riskTriggerBasis||'—')}</div></div>`+
      `<div class=source-note><b>執行時間</b><div class=mini>${esc(session)}。本頁用完成交易日資料形成條件；沒有合法即時行情時，不宣稱知道盤中現在價格。</div></div><div class=disclaimer><b>信心指數不是勝率</b>StockLab 的「決策信心指數」只由 9+3 已驗證資料覆蓋率與方向一致性計算，不代表未來上漲／下跌機率。若 9+3 不完整，方向結論維持不完整，但合法價格結構仍可提供條件式風險線；若合法歷史本身不足，才不產生數字風險線。任何缺值都不回退到前日低點、固定百分比或 AI 猜值。</div>`;
  }
