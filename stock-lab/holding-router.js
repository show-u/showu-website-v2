...
(function(){
  ...
  function positionFacts(code,name,p){
    ...
    return `<div class=toprow><div><h2>${esc(name?`${name}／${code}`:code)}</h2><div class=muted>已持有｜部位管理與出場條件</div></div></div>
      <h3>你的持股事實</h3><div class=sourcegrid>
      <div class=sourceitem><b>成本均價｜user_observed</b>${money(p.averageCost)}</div>
      <div class=sourceitem><b>目前持有股數｜user_observed</b>${money(p.shares)}</div>
      <div class=sourceitem><b>首次買入日｜${p.buyDate?'user_observed':'unavailable'}</b>${p.buyDate||MISSING}</div>
      <div class=sourceitem><b>輸入成本合計｜derived</b>${money(inputCost)}</div></div>`;
  }
  ...
  async function localHolding(code,p,name,market){
    ...
    let vf;if(market==='TPEx'){
      const snap=await window.StockLabTPEx?.tpexSnapshot?.(),row=(snap||[]).find(x=>String(x.code)===code);vf=await window.StockLabTPEx?.verifyTpex?.(code,bars,row);
    }else vf=await verify(code,bars);
  ...
  function renderPrivate(code,p,j){
    ...
    const d=j?.data||{},name=d.name||'',facts=positionFacts(code,name,p);
    ...
    box.innerHTML=facts+`<div class=source-note><b>${esc(d.action||'持股狀態')}｜model_estimate</b><div class=mini>資料日 ${esc(d.data_date||'—')}｜模型 ${esc(j.model_version||'—')}<br>${esc(d.reason||'')}</div></div>`;
  }
