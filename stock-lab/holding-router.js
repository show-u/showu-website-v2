...
(function(){
  ...
  async function localHolding(code,p,name,market){
    ...
    box.innerHTML=positionFacts(code,name,p)+`<div class=decision-strip><div class=sourceitem><div class=hero-label>目前建議狀態</div><div class=hero-number style="font-size:24px">${esc(q.state||'持股判斷')}</div><div class=mini>${esc(q.reason||'')}</div></div><div class=sourceitem><div class=hero-label>${esc(ref.label)}</div><div class=hero-number>${Number.isFinite(Number(ref.value))?money(ref.value):MISSING}</div><div class=mini>${esc(ref.condition)}</div></div></div><div class=sourcegrid style="margin-top:8px"><div class=sourceitem><b>最新官方收盤</b>${esc(result.dataDate)}｜${money(result.latestClose)}</div><div class=sourceitem><b>目前市值</b>${Number.isFinite(Number(d.marketValue))?money(d.marketValue):MISSING}</div><div class=sourceitem><b>未實現損益</b>${pnlText}</div><div class=sourceitem><b>已持有時間</b>${holdingDays}</div><div class=sourceitem><b>信心指數</b>${Math.round(ci)}/100<br><span class=mini>已校準；不是成交機率</span></div><div class=sourceitem><b>執行時點</b>以完成交易日收盤確認；符合條件後於下一合法交易時段重新判定</div></div><div class=source-note><b>出場判斷方式</b><div class=mini>先看條件是否成立，再看觸發參考價；不因價格碰到某一數字就自動視為必須賣出，也不把事後高低點當成可成交價格。</div></div>`;
  }
  function renderPrivate(code,p,j){
    ...
    box.innerHTML=facts+`<div class=decision-strip><div class=sourceitem><div class=hero-label>目前建議狀態</div><div class=hero-number style="font-size:24px">${esc(d.action||'持股判斷')}</div><div class=mini>${esc(d.reason||'')}</div></div><div class=sourceitem><div class=hero-label>出場觸發參考價${hasRange?'區間':''}</div><div class=hero-number>${exitText}</div><div class=mini>${esc(d.exit_condition||'依模型條件於下一合法交易時段重新確認')}</div></div></div><div class=sourcegrid style="margin-top:8px"><div class=sourceitem><b>最新官方收盤</b>${esc(d.data_date||'—')}${d.close!=null?`｜${money(d.close)}`:''}</div><div class=sourceitem><b>目前市值</b>${d.market_value!=null?money(d.market_value):MISSING}</div><div class=sourceitem><b>未實現損益</b>${pnl}｜${pct}</div><div class=sourceitem><b>已持有時間</b>${holdingDays}</div><div class=sourceitem><b>信心指數</b>${Math.round(ci)}/100<br><span class=mini>已校準；不是成交機率</span></div><div class=sourceitem><b>執行原則</b>條件優先，價格次之；完成交易日確認後於下一合法交易時段執行</div></div><div class=source-note><b>出場判斷方式</b><div class=mini>觸發參考價必須搭配趨勢、風險狀態與模型條件，不是單一固定賣價或保證成交價。</div></div>`
  }
  btn.onclick=async()=>{
