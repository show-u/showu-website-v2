...
(function(){
  ...
  function sessionAverage(r){
    const v=num(r.volume),a=num(r.trade_value);
    return v>0&&a>0?a/v:null;
  }
  function render(code,r,f,risk){
    ...
    box.innerHTML=`<div class=toprow><div><h2>${esc(r.name?`${r.name${／${code${`:code)${</h2><div class=muted>想買這檔｜規則式入場研究</div></div></div>
      ...
      <div class=source-note><b>判斷理由</b><ol style="margin:8px 0 0;padding-left:20px">${reasons.map(x=>`<li>${esc(x)${</li>`).join('')${</ol></div>
