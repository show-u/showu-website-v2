...
(async()=>{
 ...
 function makeObs(date,worst=true){
   const rows=snapshot(date,worst);
   if(rows.length<80)return [];
   const pr=pct(rows.map(x=>x.m));
   return rows.map((x,i)=>({...x,date,pct:pr[i],winner:x.ret>=.20?1:0}));
 }
 const obs=dates.flatMap(d=>makeObs(d,true));
 const train=obs.filter(x=>x.date<'2024-01-01');
 const test=obs.filter(x=>x.date>='2024-01-01');

 // Frozen individual-stock procedure:
 // 1) use MOM6 cross-sectional percentile only;
 // 2) five fixed quintile bins;
 // 3) estimate probability in training period with Laplace smoothing;
 // 4) apply unchanged to locked 2024-2026 test period;
 // 5) assess calibration + discrimination versus unconditional base rate.
 const bins=[[0,.2],[.2,.4],[.4,.6],[.6,.8],[.8,1.0000001]];
 const trainStats=bins.map(([lo,hi],idx)=>{
   const a=train.filter(x=>x.pct>=lo&&x.pct<hi),w=a.reduce((s,x)=>s+x.winner,0);
   return {bin:idx+1,lo,hi,n:a.length,winners:w,raw:w/a.length,p:(w+1)/(a.length+2)};
 });
 const pFor=x=>trainStats[Math.min(4,Math.floor(x.pct*5))].p;
 const baseTrain=(train.reduce((s,x)=>s+x.winner,0)+1)/(train.length+2);
 const baseTest=mean(test.map(x=>x.winner));

 function auc(rows){
   const pos=rows.filter(x=>x.winner),neg=rows.filter(x=>!x.winner);if(!pos.length||!neg.length)return null;
   let wins=0,ties=0;for(const a of pos)for(const b of neg){if(a.pct>b.pct)wins++;else if(a.pct===b.pct)ties++}
   return (wins+.5*ties)/(pos.length*neg.length);
 }
 function brier(rows,useModel=true){
   return mean(rows.map(x=>{const p=useModel?pFor(x):baseTrain;return (p-x.winner)**2}));
 }
 function logloss(rows,useModel=true){
   return -mean(rows.map(x=>{let p=useModel?pFor(x):baseTrain;p=Math.min(.999999,Math.max(.000001,p));return x.winner*Math.log(p)+(1-x.winner)*Math.log(1-p)}));
 }
 const testBins=bins.map(([lo,hi],idx)=>{
   const a=test.filter(x=>x.pct>=lo&&x.pct<hi);
   return {bin:idx+1,lo,hi,n:a.length,predicted:trainStats[idx].p,actual:mean(a.map(x=>x.winner)),meanReturn:mean(a.map(x=>x.ret)),medianReturn:median(a.map(x=>x.ret))};
 });
 const top20=test.filter(x=>x.pct>=.8),bottom80=test.filter(x=>x.pct<.8);
 const top10=test.filter(x=>x.pct>=.9),bottom90=test.filter(x=>x.pct<.9);
 const diff20=(()=>{
   const by=[];
   for(const d of [...new Set(test.map(x=>x.date))]){
     const a=test.filter(x=>x.date===d),t=a.filter(x=>x.pct>=.8),b=a.filter(x=>x.pct<.8);
     if(t.length&&b.length)by.push(mean(t.map(x=>x.winner))-mean(b.map(x=>x.winner)));
   }
   return {n:by.length,mean:mean(by),positive:by.filter(x=>x>0).length/by.length,p:perm(by),ci:boot(by)};
 })();
 const diff10=(()=>{
   const by=[];
   for(const d of [...new Set(test.map(x=>x.date))]){
     const a=test.filter(x=>x.date===d),t=a.filter(x=>x.pct>=.9),b=a.filter(x=>x.pct<.9);
     if(t.length&&b.length)by.push(mean(t.map(x=>x.winner))-mean(b.map(x=>x.winner)));
   }
   return {n:by.length,mean:mean(by),positive:by.filter(x=>x>0).length/by.length,p:perm(by),ci:boot(by)};
 })();

 // Stability by year in locked test.
 const yearly={};
 for(const y of ['2024','2025','2026']){
   const a=test.filter(x=>x.date.startsWith(y));if(!a.length)continue;
   yearly[y]={n:a.length,base:mean(a.map(x=>x.winner)),top20:mean(a.filter(x=>x.pct>=.8).map(x=>x.winner)),auc:auc(a),brierModel:brier(a,true),brierBase:brier(a,false)};
 }

 // Simple decision labels are descriptive probability bands, not buy/sell recommendations.
 const labelFor=p=>p>=.25?'A_high_candidate':p>=.18?'B_above_average':p>=.12?'C_average':'D_below_average';
 const scorecard=trainStats.map(x=>({...x,label:labelFor(x.p)}));
 const result={
   protocol:{
     target:'future 84 trading-day return >=20%',
     predictor:'MOM6 cross-sectional percentile only',
     calibration:'fixed quintiles trained on 2021-2023; locked test 2024-2026',
     universe:'150 fresh current stocks + official 2021-2026 TWSE/TPEx delistings',
     delistTreatment:'-100% if delisted before completing horizon',
     output:'probability band / candidate strength, not direction guarantee'
   },
   counts:{train:train.length,test:test.length,trainDates:new Set(train.map(x=>x.date)).size,testDates:new Set(test.map(x=>x.date)).size},
   trainBase:baseTrain,testBase:baseTest,
   scorecard,
   testBins,
   discrimination:{auc:auc(test),top20VsRest:diff20,top10VsRest:diff10},
   calibration:{
     brierModel:brier(test,true),brierBase:brier(test,false),
     brierSkill:1-brier(test,true)/brier(test,false),
     loglossModel:logloss(test,true),loglossBase:logloss(test,false)
   },
   pooled:{
     top20:{n:top20.length,hit:mean(top20.map(x=>x.winner)),meanRet:mean(top20.map(x=>x.ret)),medianRet:median(top20.map(x=>x.ret))},
     rest80:{n:bottom80.length,hit:mean(bottom80.map(x=>x.winner))},
     top10:{n:top10.length,hit:mean(top10.map(x=>x.winner)),meanRet:mean(top10.map(x=>x.ret)),medianRet:median(top10.map(x=>x.ret))},
     rest90:{n:bottom90.length,hit:mean(bottom90.map(x=>x.winner))}
   },
   yearly
 };
