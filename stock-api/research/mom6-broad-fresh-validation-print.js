...
(async()=>{
 ...
 console.log('LOCK',JSON.stringify({freshSurvivors:Object.keys(surv).length,delisted:dels.length,total:Object.keys(data).length,codes:Object.keys(surv)}));
 ...
 function evalRule(frac=.2,cost=0,thr=.2,sectorNeutral=false,worst=false){
  const per=[];let tot=0,w=0,seln=0,selw=0;
  for(const date of dates){const rows=snapshot(date,worst);if(rows.length<80)continue;
   let score=new Map();
   if(sectorNeutral){const groups=new Map();for(const x of rows){if(!groups.has(x.industry))groups.set(x.industry,[]);groups.get(x.industry).push(x)}
    for(const g of groups.values()){const pr=pct(g.map(x=>x.m));g.forEach((x,i)=>score.set(x.code,pr[i]));}
   } else {const pr=pct(rows.map(x=>x.m));rows.forEach((x,i)=>score.set(x.code,pr[i]));}
   const k=Math.max(5,Math.floor(rows.length*frac)),sel=[...rows].sort((a,b)=>score.get(b.code)-score.get(a.code)).slice(0,k);
   const br=mean(rows.map(x=>x.ret>=thr?1:0)),pr=mean(sel.map(x=>x.ret-cost>=thr?1:0)),lift=pr-br;
   per.push({date,n:rows.length,k,baseRate:br,precision:pr,lift,delistedSelected:sel.filter(x=>x.delisted).length});
   tot+=rows.length;w+=rows.filter(x=>x.ret>=thr).length;seln+=sel.length;selw+=sel.filter(x=>x.ret-cost>=thr).length;
  }
  const L=per.map(x=>x.lift);return{nDates:per.length,avgN:mean(per.map(x=>x.n)),baseRate:mean(per.map(x=>x.baseRate)),precision:mean(per.map(x=>x.precision)),lift:mean(L),positive:L.filter(x=>x>0).length/L.length,p:perm(L),ci:boot(L),pooledBase:w/tot,pooledPrecision:selw/seln,delistedSelections:per.reduce((s,x)=>s+x.delistedSelected,0),perDate:per};
 }
 const result={
  raw10:evalRule(.10,0,.20,false,true),
  raw20:evalRule(.20,0,.20,false,true),
  raw30:evalRule(.30,0,.20,false,true),
  sector20:evalRule(.20,0,.20,true,true),
  cost1pct:evalRule(.20,.01,.20,false,true),
  win15:evalRule(.20,0,.15,false,true),
  win30:evalRule(.20,0,.30,false,true)
 };
 const split={};for(const [k,v] of Object.entries(result)){const early=v.perDate.filter(x=>x.date<'2024-01-01').map(x=>x.lift),late=v.perDate.filter(x=>x.date>='2024-01-01').map(x=>x.lift);split[k]={early:{n:early.length,mean:mean(early),p:perm(early),ci:boot(early)},late:{n:late.length,mean:mean(late),p:perm(late),ci:boot(late)}}}
 console.log('RESULT',JSON.stringify({result,split}));
