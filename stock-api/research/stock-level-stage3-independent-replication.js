...
(async()=>{
 ...
 const dif={stage2:[],ret20:[],highProx:[],mom6deep:[],ret20_highProx:[],ret20_mom6:[],all3:[]};
 const precisions={stage2:[],ret20:[],highProx:[],mom6deep:[],ret20_highProx:[],ret20_mom6:[],all3:[]};
 const counts={stage2:[],ret20:[],highProx:[],mom6deep:[],ret20_highProx:[],ret20_mom6:[],all3:[]};
 const rowsOut=[];
 for(const date of dates){
  ...
  const psMed=q(mom.map(x=>x.posShare),.5),stage2=mom.filter(x=>x.posShare>=psMed);if(stage2.length<10)continue;
  const pMom=mean(mom.map(x=>x.ret>=.2?1:0)),p2=mean(stage2.map(x=>x.ret>=.2?1:0));
  const r20Med=q(stage2.map(x=>x.ret20),.5),hpMed=q(stage2.map(x=>x.highProx),.5),m6Med=q(stage2.map(x=>x.mom6),.5);
  const groups={
    stage2,
    ret20:stage2.filter(x=>x.ret20>=r20Med),
    highProx:stage2.filter(x=>x.highProx>=hpMed),
    mom6deep:stage2.filter(x=>x.mom6>=m6Med),
    ret20_highProx:stage2.filter(x=>x.ret20>=r20Med&&x.highProx>=hpMed),
    ret20_mom6:stage2.filter(x=>x.ret20>=r20Med&&x.mom6>=m6Med),
    all3:stage2.filter(x=>x.ret20>=r20Med&&x.highProx>=hpMed&&x.mom6>=m6Med)
  };
  const rec={date,n:rows.length,momN:mom.length,momPrecision:pMom,groups:{}};
  for(const [k,g] of Object.entries(groups)){
    if(g.length<3)continue;
    const pr=mean(g.map(x=>x.ret>=.2?1:0));
    dif[k].push(pr-pMom);precisions[k].push(pr);counts[k].push(g.length);
    rec.groups[k]={n:g.length,precision:pr,deltaVsMOM20:pr-pMom};
  }
  rowsOut.push(rec);
 }
 const summary={};
 for(const k of Object.keys(dif)){
   const v=dif[k];
   summary[k]={n:v.length,avgN:mean(counts[k]),precision:mean(precisions[k]),deltaVsMOM20:mean(v),positive:v.filter(x=>x>0).length/v.length,p:perm(v),ci:boot(v)};
 }
 console.log('RESULT',JSON.stringify({summary,byDate:rowsOut,
  pass:{
   stage2:summary.stage2.deltaVsMOM20>0&&summary.stage2.p<.05&&summary.stage2.ci[0]>0,
   ret20:summary.ret20.deltaVsMOM20>0&&summary.ret20.p<.05&&summary.ret20.ci[0]>0,
   highProx:summary.highProx.deltaVsMOM20>0&&summary.highProx.p<.05&&summary.highProx.ci[0]>0,
   mom6deep:summary.mom6deep.deltaVsMOM20>0&&summary.mom6deep.p<.05&&summary.mom6deep.ci[0]>0
  }}));
