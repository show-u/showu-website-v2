...
(async()=>{
 console.log('PROTOCOL',JSON.stringify({
  objective:'Final confirmatory head-to-head: RIGHTTAIL_5 versus simple 6-to-1 momentum',
 ...
 const all=evalRule(primary,snapshots);

 // Sensitivity only, pre-specified: winner thresholds 15%, 20% primary, 30%.
 function evalThreshold(thr){
   const per=[];let totalW=0,caught=0,totalSel=0,totalSelW=0;
   for(const s of snapshots){
     const r=s.rows.filter(x=>Number.isFinite(x[primary.name+'_S']));if(r.length<40)continue;
     const k=Math.max(5,Math.floor(r.length*.2)),sel=[...r].sort((a,b)=>b[primary.name+'_S']-a[primary.name+'_S']).slice(0,k);
     const br=mean(r.map(x=>x.ret>=thr?1:0)),pr=mean(sel.map(x=>x.ret>=thr?1:0)),lift=pr-br;
     totalW+=r.filter(x=>x.ret>=thr).length;caught+=sel.filter(x=>x.ret>=thr).length;totalSel+=sel.length;totalSelW+=sel.filter(x=>x.ret>=thr).length;
     per.push({date:s.date,baseRate:br,precision:pr,lift});
   }
   const L=per.map(x=>x.lift);
   return {threshold:thr,nDates:per.length,baseRate:mean(per.map(x=>x.baseRate)),precision:mean(per.map(x=>x.precision)),lift:mean(L),positive:L.filter(x=>x>0).length/L.length,p:perm(L),ci:boot(L),recall:totalW?caught/totalW:null,pooledPrecision:totalSel?totalSelW/totalSel:null};
 }
 const comps={};for(const r of RULES.slice(1))comps[r.name]=evalRule(r,snapshots);
 const mom=comps.MOM6;
 const paired=[];for(let i=0;i<Math.min(all.perDate.length,mom.perDate.length);i++){
   if(all.perDate[i].date===mom.perDate[i].date)paired.push(all.perDate[i].lift-mom.perDate[i].lift);
 }
 const headToHead={n:paired.length,meanLiftAdvantage:mean(paired),positive:paired.filter(x=>x>0).length/paired.length,p:perm(paired),ci:boot(paired)};
 const pass=all.winnerLift>0&&all.permP<.05&&all.ci[0]>0&&all.winnerLiftPositive>=.6&&all.pooledPrecision>all.avgBaseRate;
 console.log('RESULT',JSON.stringify({
   nStocks:Object.keys(data).length,nDates:snapshots.length,
   primaryRule:'RIGHTTAIL_5',
   primary:all,
   sensitivity:[evalThreshold(.15),evalThreshold(.20),evalThreshold(.30)],
   comparators:comps,
   headToHeadVsMOM6:headToHead,
   pass,
   status:pass?'REPLICATED':'NOT_REPLICATED'
 }));
