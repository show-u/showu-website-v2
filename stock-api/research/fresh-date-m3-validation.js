...
const START='2021-01-01',END='2026-09-22',K=30,EMBARGO=20,DIVERSIFY=10,SEED=20260922;
const EXCLUDE=['2024-08-21','2025-08-21','2026-08-21'];
...
(async()=>{
  ...
  for(const s of stocks){
    ...
    data[s.code]=rows; commonSets.push(new Set(rows.map(r=>r.date)));
  }
  const common=[...commonSets[0]].filter(d=>commonSets.every(s=>s.has(d))).sort();
  const pos=new Map(common.map((d,i)=>[d,i]));
  const excluded=new Set();
  for(const e of EXCLUDE){
    const p=pos.get(e); if(p==null)continue;
    for(let j=Math.max(0,p-20);j<=Math.min(common.length-1,p+20);j++)excluded.add(common[j]);
  }
  const eligible=common.filter(d=>{
    const y=Number(d.slice(0,4));
    if(y<2023||y>2026||excluded.has(d))return false;
    const p=pos.get(d);
    if(p<300||p+20>=common.length)return false;
    return true;
  });
  const r=rng(SEED),shuffled=[...eligible].sort(()=>r()-.5);
  const chosen=[];
  for(const d of shuffled){
    const p=pos.get(d);
    if(chosen.every(x=>Math.abs(pos.get(x)-p)>=40)){chosen.push(d);if(chosen.length===12)break}
  }
  chosen.sort();
  console.log('LOCKED_DATES',JSON.stringify({seed:SEED,dates:chosen,excludedOriginalDates:EXCLUDE,exclusionWindowTradingDays:20,minSpacingTradingDays:40}));

  const summary={M3:{5:[],10:[],20:[]},ALWAYS_UP:{5:[],10:[],20:[]},TREND:{5:[],10:[],20:[]}};
  for(const snap of chosen){
    for(const s of stocks){
      const p=prep(data[s.code],snap),m3=selectM3(p);
      for(const h of [5,10,20]){
        const actual=sgn(p.target.f[h]);
        const preds={
          M3:sgn(median(m3.map(x=>x.f[h]))),
          ALWAYS_UP:1,
          TREND:sgn(p.target.x[3])
        };
        for(const m of Object.keys(preds))summary[m][h].push(preds[m]===actual?1:0);
      }
    }
  }
  for(const m of Object.keys(summary))for(const h of [5,10,20]){
    const a=summary[m][h];
    console.log('RESULT',JSON.stringify({model:m,horizon:h,n:a.length,correct:a.reduce((x,y)=>x+y,0),accuracy:mean(a)}));
  }
