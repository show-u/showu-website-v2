...
async function twseDelisted(){
 ...
 const j=await r.json();return (j.data||[]).map(x=>{const [roc,name,code]=x,parts=roc.split('/').map(Number);return {code,name,date:(parts[0]+1911)+'-'+String(parts[1]).padStart(2,'0')+'-'+String(parts[2]).padStart(2,'0')}})
  .filter(x=>/^[0-9]{4}$/.test(x.code)&&x.date>='2021-01-01'&&x.date<='2026-09-22');
}
function mom6(p,i){return i>=126?p[i-21].close/p[i-126].close-1:null}
(async()=>{
 console.log('PROTOCOL',JSON.stringify({
  objective:'Survivorship-bias stress test of MOM6 right-tail selection',
  universe:'same 60 fresh stocks plus every 4-digit TWSE company officially delisted during 2021-2026 with FinMind historical prices',
 ...
 const dels=await twseDelisted();console.log('OFFICIAL_DELISTED',JSON.stringify(dels));
 ...
 console.log('RESULT',JSON.stringify({
  ...
  residualLimitation:'TPEx delisted-company list was not incorporated in this stress run; TWSE delistings are official and complete for the tested period.'
 }));
