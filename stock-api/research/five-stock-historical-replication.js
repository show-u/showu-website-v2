...
const SNAPSHOTS=['2024-08-21','2025-08-21'];
const START='2021-01-01', END='2026-09-22';
...
function stateAt(cut,rm){
  ...
  return {
    snapshotDate:cut[i].date,price:cut[i].c,r20,
    momentumPercentile:mp,mstate,
    turnover5:t5,turnoverRatio:ratio,turnoverPercentile:tp,tstate,
    market:m.trend+'_'+m.vol,taiex:m.close,taiexMA120:m.ma120,taiexRV20:m.rv20
  };
}
function futureReturns(full,snapDate){
  const i=full.findIndex(x=>x.date===snapDate);
  if(i<0)throw Error('snapshot missing');
  const base=full[i].c,out={};
...
(async()=>{
  console.log('LOCKED PARAMETERS',JSON.stringify({
    snapshots:SNAPSHOTS,
    stocks:stocks.map(x=>x.code),
    horizons:[1,3,5,10,20],
    momentumWindow:20,momentumHistory:252,
    turnoverAvg:5,turnoverMedian:60,turnoverHistory:120,
    percentiles:[.2,.8],marketMA:120,marketRV:20,
    antiLookahead:'feature data <= each snapshot date only; outcomes revealed afterward',
    noDateSubstitution:true,noParameterChanges:true
  },null,2));

  const [idxAll,...stockData]=await Promise.all([
    fm('TaiwanStockTotalReturnIndex','TAIEX'),
    ...stocks.flatMap(s=>[
  ...
  for(const s of stocks){
    data[s.code]=buildSeries(stockData[k++],stockData[k++]);
  }

  for(const snap of SNAPSHOTS){
    console.log('\nYEAR',snap);
    const idxCut=idxAll.filter(x=>x.date<=snap);
    const rm=regimeMap(idxCut);
    const yearRows=[];
    for(const s of stocks){
      const full=data[s.code];
      const cut=full.filter(x=>x.date<=snap);
      if(cut.at(-1)?.date!==snap)throw Error(s.code+' '+snap+' is not a trading day');
      const snapshot=stateAt(cut,rm);
      const future=futureReturns(full,snap);
      const row={code:s.code,name:s.name,...snapshot,future};
      yearRows.push(row);
      console.log('REPLAY',JSON.stringify(row));
    }
    for(const h of [1,3,5,10,20]){
      const vals=yearRows.map(x=>x.future[h]?.ret).filter(Number.isFinite);
      console.log('YEAR_SUMMARY',JSON.stringify({
        snapshot:snap,horizon:h,n:vals.length,
        positive:vals.filter(x=>x>0).length,
